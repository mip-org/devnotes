...
> Path conventions in this note: `tests/…` and `match_build.m`/`read_mip_yaml.m`
> refer to the [mip](https://github.com/mip-org/mip) repo;
> `tools/src/mip_channel_tools/…` refers to the Python package now at
> `src/mip_channel_tools/` in
> [mip_channel_tools](https://github.com/mip-org/mip_channel_tools), which also
> owns `build-package.yml`.
